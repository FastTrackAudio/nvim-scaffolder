...
MiniSnippets.default_match = function(snippets, opts)
  ...
  opts = vim.tbl_extend(
    'force',
    { pattern_exact_boundary = '[%s%p]?', pattern_fuzzy = '%S*' },
    opts or {}
  )
  ...
  for i, s in pairs(snippets) do
    ...
    if
      best_match_width < w
      and line:sub(-w) == s.prefix
      and line:sub(-w - 1, -w - 1):find(pattern_boundary)
    then
  ...
  if best_id ~= nil then
    ...
    res.region =
      { from = { line = lnum, col = to - best_match_width + 1 }, to = { line = lnum, col = to } }
    return { res }
  end
...
H.create_default_hl = function()
  local hi_link_underdouble = function(to, from)
    local data = vim.fn.has('nvim-0.9') == 1
        and vim.api.nvim_get_hl(0, { name = from, link = false })
  ...
  hi_link_underdouble(
    'MiniSnippetsFinal',
    'DiagnosticUnderline' .. (vim.fn.has('nvim-0.9') == 1 and 'Ok' or 'Hint')
  )
...
H.parse_normalize = function(node_arr, opts)
  ...
  local sync_linked_tabstops = function(n)
    ...
    if ref ~= nil then
      ...
      n.text, n.placeholder, n.choices =
        ref.text, vim.deepcopy(ref.placeholder), vim.deepcopy(ref.choices)
      return
    end
...
H.parse_processors.dollar_lbrace = function(c, s, n)
  if n.tabstop == nil and n.var == nil then -- Detect the type of `${...}`
    ...
    H.error(
      '`${` should be followed by digit (in tabstop) or letter/underscore (in variable), not '
        .. vim.inspect(c)
    )
  end
  if c == '}' then
...
H.session_init = function(session, full)
  ...
  H.trigger_event(
    'MiniSnippetsSession' .. (full and 'Start' or 'Resume'),
...
H.track_sessions = function()
  ...
  local on_textchanged = function(args)
    ...
    if not H.session_is_valid(session) then
      H.notify(
        'Session contains corrupted data (deleted or out of range extmarks). It is stopped.',
        'WARN'
      )
  ...
  vim.api.nvim_create_autocmd(
    text_events,
    { group = gr, callback = on_textchanged, desc = 'React to text change' }
  )
  ...
  local modechanged_opts =
    { group = gr, pattern = '*:n', callback = stop_if_final, desc = 'Stop on final tabstop' }
  vim.api.nvim_create_autocmd('ModeChanged', modechanged_opts)
  vim.api.nvim_create_autocmd(
    text_events,
    { group = gr, callback = stop_if_final, desc = 'Stop on final tabstop' }
  )
end
...
H.session_update_hl = function(session)
  ...
  local empty_tabstop, empty_tabstop_final =
    insert_opts.empty_tabstop, insert_opts.empty_tabstop_final
  ...
  local update_hl = function(n, is_in_cur_tabstop)
    ...
    local hl_group = (n.tabstop == cur_tabstop or is_in_cur_tabstop) and current_hl
      or (
        is_final and 'MiniSnippetsFinal'
        or (is_visited and 'MiniSnippetsVisited' or 'MiniSnippetsUnvisited')
      )
...
H.session_deinit = function(session, full)
  ...
  -- Trigger proper event
  H.trigger_event(
    'MiniSnippetsSession' .. (full and 'Stop' or 'Suspend'),
    { session = vim.deepcopy(session) }
  )
...
H.call_in_insert_mode = function(f)
  ...
  vim.api.nvim_create_autocmd(
    'ModeChanged',
    { pattern = '*:i*', once = true, callback = cb, desc = 'Call in Insert' }
  )
end

  ...
  for _, arr in ipairs({ move, rename, copy, create }) do
    for _, diff in ipairs(arr) do
      ...
      for _, del in ipairs(delete) do
        local from_is_affected = del.from == diff.from
          or vim.startswith(diff.from or '', del.from .. '/')
...
H.explorer_ignore_pending_fs_actions = function(explorer, action_name)
  ...
  local msg = string.format(
    'There are pending file system actions\n\n%s without synchronization?',
    action_name
  )
  local confirm_res = vim.fn.confirm(msg, '&Yes\n&No', 1, 'Question')
...
H.view_ensure_proper = function(view, path, opts, is_focused, is_preview)
  ...
  local needs_recreate, needs_reprocess =
    not H.is_valid_buf(view.buf_id), not view.was_focused and is_focused
...
H.buffer_make_mappings = function(buf_id, mappings)
  ...
  local mark_goto = function()
    ...
    local is_valid_path = type(path) == 'string'
      and H.fs_get_type(vim.fn.expand(path)) == 'directory'
    if not is_valid_path then
...
H.buffer_update_directory = function(buf_id, path, opts, is_preview)
  ...
  local prefix_fun, n_computed_prefixes =
    opts.content.prefix, is_preview and vim.o.lines or math.huge
...
H.validate_branch = function(x)
  ...
  for i = 2, #res do
    ...
    if (parent .. '/' .. child:match('[^/]+$')) ~= res[i] then
      H.error(
        '`branch` contains not a parent-child pair: '
          .. vim.inspect(parent)
          .. ' and '
          .. vim.inspect(child)
      )
...
H.fit_to_width = function(text, width)
  ...
  return t_width <= width and text
    or ('…' .. vim.fn.strcharpart(text, t_width - width + 1, width - 1))
end
